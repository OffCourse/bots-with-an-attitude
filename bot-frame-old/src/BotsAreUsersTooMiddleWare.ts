  ...
  public async onTurn(
    ...
    if (context.activity.type === "message" && context.activity.channelData) {
      ...
      if (context.activity.channelData.type == "block_actions") {
        ...
        context.activity.channelData.channel_type &&
        context.activity.channelData.channel_type === "im"
      ) {
        ...
        context.activity.text.match(direct_mention)
      ) {
        ...
        bot_user_id &&
        context.activity.text &&
        context.activity.text.match(mention)
      ) {
...
export class BotsAreUsersTooMiddleWare extends MiddlewareSet {
  public async onTurn(
    ...
    // @ts-ignore
    const info = await adapter.slack.bots.info({
      bot: context.activity.channelData.bot_id
    });
    const bot_user_id = await adapter.getBotUserByTeam(context.activity);
